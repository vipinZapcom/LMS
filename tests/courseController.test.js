...
describe("Course Controller Unit Tests", () => {
  ...
  it("should create a course successfully", async () => {
    ...
    Course.prototype.save = jest
      .fn()
      .mockResolvedValue({ ...courseData, uploadedBy: userId });
  ...
  it("should get course by id", async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining({ success: true, data: mockCourse })
    );
  });
  ...
  it("should get all courses", async () => {
    ...
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith(
      expect.objectContaining({ success: true, data: courses })
    );
  });
  ...
  it("should return 403 if user is not uploader", async () => {
    const mockCourse = { title: "Old", uploadedBy: "otherUser",instructor: "mockUserId1234", };
    req.params = { id: "1" };
    req.body = { title: "Updated" };
    req.user = { id: userId };
    Course.findById.mockResolvedValue(mockCourse);
  ...
  it("should assign course to groups successfully", async () => {
    ...
    await courseController.assignCourseToGroups(req, res);
  ...
  it("should return 400 if courseId or groupIds missing", async () => {
    ...
    expect(res.json).toHaveBeenCalledWith({
      success: false,
      message: "courseId and groupIds array are required",
    });
  });
